...
const Login = ({ setCurrUser, setShow }) => {
  ...
  const login = async (userInfo) => {
    try {
      ...
      // Almacena el token en el estado o localStorage
      localStorage.setItem("token", data.token);

      // Actualiza el estado del usuario o realiza otras acciones necesarias
      setCurrUser({
        name: data.name,
        email: data.email,
        // Otras propiedades según la estructura de tu usuario
      });
    } catch (error) {
